...
//define hexbug
String hexBug_portName = "/dev/tty.usbmodem1451";  //starts as N/A but is selected from control panel to match your OpenBCI USB Dongle's serial/COM
...
public void systemDraw(){ //for drawing to the screen
  ...
  if(systemMode == 10){
    ...
    if ((redrawScreenNow) || (drawLoop_counter >= drawLoopCounter_thresh)) {
      ...
      switch (eegDataSource) {
        case DATASOURCE_NORMAL: case DATASOURCE_NORMAL_W_AUX:
          frame.setTitle(PApplet.parseInt(frameRate) + " fps, Byte Count = " + openBCI_byteCount + ", bit rate = " + byteRate_perSec*8 + " bps" + ", " + PApplet.parseInt(PApplet.parseFloat(fileoutput.getRowsWritten())/openBCI.get_fs_Hz()) + " secs Saved, Writing to " + output_fname);
          break;
        case DATASOURCE_SYNTHETIC:
          frame.setTitle(PApplet.parseInt(frameRate) + " fps, Using Synthetic EEG Data");
          break;
        case DATASOURCE_PLAYBACKFILE:
          frame.setTitle(PApplet.parseInt(frameRate) + " fps, Playing " + PApplet.parseInt(PApplet.parseFloat(currentTableRowIndex)/openBCI.get_fs_Hz()) + " of " + PApplet.parseInt(PApplet.parseFloat(playbackData_table.getRowCount())/openBCI.get_fs_Hz()) + " secs, Reading from: " + playbackData_fname);
          break;
      } 
    }
...
public void sendToUnity() {
  OscMessage myOscMessage = new OscMessage("/OpenBCITest");  
...
class EEG_Processing_User {
  ...
  public void applyDetectionRules_2D(DetectedPeak[] peakPerBand, DetectedPeak detectedPeak) {
    int band_A = 0, band_B = 1, band_C = 2, band_D = 3;
    int nRules = 3;
    float[] value_from_each_rule = new float[nRules];
    float primary_value_dB=0.0f, secondary_value_dB=0.0f;
    int nDetect = 0;
    ...
    peakPerBand[band_D].threshold_dB = 4.5f * sqrt(abs(1.1f - pow(primary_value_dB/detection_thresh_dB, 2.0f)));
    ...
    peakPerBand[band_C].threshold_dB = 3.0f;
    secondary_value_dB = peakPerBand[band_D].SNR_dB;
    final float slope = (7.5f-(-3))/(12-4);
    final float yoffset = 7.5f - slope*12;
  ...
  public void updateGUI_FFTPlot(Blank2DTrace.PlotRenderer pr) {
    float x1, y1, x2, y2;
    boolean is_detected;
    ...
    if (showDetectionOnGUI) {
      ...
      for (int Iband=2; Iband<3; Iband++) { // only plot Forward band
        ...
        //add label
        x1 = pr.valToX(0.5f*(processing_band_low_Hz[Iband]+processing_band_high_Hz[Iband]));
        y1 = pr.valToY(60.0f);
      ...
      for (int Ichan= (2-1); Ichan < 8; Ichan += 2) {
        ...
        if (useClassfier_2DTraining) {
          ...
          for (int Iband=0; Iband<peakPerBand.length; Iband++) {
            ...
            float val_uV_perBin = detectedPeak[Ichan].background_rms_uV_perBin * sqrt(pow(10.0f, thresh_dB/10.0f));
  ...
  public void addMarker(Blank2DTrace.PlotRenderer pr, float new_x2, float new_y2, boolean is_detected) {
    int diam = 8;
    pr.canvas.pushMatrix(); //hold on to the current canvas state
  ...
  public void addHorizDashedLine(Blank2DTrace.PlotRenderer pr, float x1, float x2, float y, int style) {
    pr.canvas.pushMatrix(); //hold on to the current canvas state
    pr.canvas.stroke(0, 0, 0);  //black
    float dx; //it'll be a dashed line, so here is how long is the dash+space, pixels
    if (style==1) {
      dx = 4;
      pr.canvas.strokeWeight(1.0f);
    } else {
      dx = 8;
      pr.canvas.strokeWeight(1.5f);
    }
    float nudge = 2;
  ...
  public void addVertDashedLine(Blank2DTrace.PlotRenderer pr, float x1, float y1, float y2, int colorCode) {
    pr.canvas.pushMatrix(); //hold on to the current canvas state
  ...
  private void defineFilters() {
    ...
    for (int Ifilt=0; Ifilt < n_filt; Ifilt++) {
      ...
      case 1:
        ...
        b2 = new double[] { 
          0.96508099f, -1.19328255f, 2.29902305f, -1.19328255f, 0.96508099f
        };
        a2 = new double[] { 
          1.0f, -1.21449348f, 2.29780334f, -1.17207163f, 0.93138168f
        }; 
...
//      String montageFilterText, String detectName) {
    ...
    println("Gui_Manager: Buttons: " + PApplet.parseInt(PApplet.parseFloat(win_x)*axisMontage_relPos[0]) + ", " + (PApplet.parseInt(PApplet.parseFloat(win_y)*axisMontage_relPos[1])-40));
...
class OpenBCI_ADS1299 {
  ...
  public void set_ADS1299_gain(int Ichan, int _gain) { //Ichan counts from zero
    if ((Ichan >= 0) && (Ichan < ADS1299_gain.length)) { 
      ...
      scale_fac_uVolts_per_count[Ichan] = ADS1299_Vref / ((float)(pow(2,23)-1)) / ADS1299_gain[Ichan]  * 1000000.0f; //ADS1299 datasheet Table 7, confirmed through experiment
    }
  }
...
//  }
  ...
  private void writeValues(int[] values, float[] scale_fac) {          
    ...
    for (int Ival = 0; Ival < nVal; Ival++) {
      output.print(", ");
      output.print(String.format("%.2f", scale_fac[Ival] * PApplet.parseFloat(values[Ival])));
    }
  }
